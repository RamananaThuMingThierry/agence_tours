...
@section('titre', isset($tour->encrypted_id) ? __('tour.edit') : __('tour.add'))
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="status" name="status" required>
                        <option value="actif" {{ isset($tour->encrypted_id) && $tour->status == 'actif' ? 'selected' : '' }}>{{ __('tour.published') }}</option>
                        <option value="inactif" {{ isset($tour->encrypted_id) && $tour->status == 'inactif' ? 'selected' : '' }}>{{ __('tour.draft') }}</option>
                    </select>
                </div>
                ...
                $.ajax({
                    ...
                    error: function (xhr) {
                        ...
                        Swal.fire({
                            title: __('form.error'),
                            text: message,
                            icon: status === 422 ? "warning" : "error", // warning pour règle métier, error sinon
                            confirmButtonColor: "#d33"
                        });
                    }
                });
